...
CREATE INDEX IF NOT EXISTS idx_job_events_status_available_created
    ON events.job_events (status, available_at, created_at);

CREATE INDEX IF NOT EXISTS idx_job_events_job_id
    ON events.job_events (job_id);

CREATE INDEX IF NOT EXISTS idx_job_events_event_type_status
    ON events.job_events (event_type, status);

DROP TRIGGER IF EXISTS trg_job_events_set_updated_at ON events.job_events;
